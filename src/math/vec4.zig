const std = @import("std");
const math = std.math;

pub const Vec4 = extern struct {
    x: f32 = 0,
    y: f32 = 0,
    z: f32 = 0,
    w: f32 = 0,

    pub fn init(x: f32, y: f32) Vec4 {
        return .{ .x = x, .y = y };
    }

    pub fn indexer(vec: Vec4, comptime index: comptime_int) f32 {
        switch (index) {
            0 => return vec.x,
            1 => return vec.y,
            2 => return vec.z,
            else => @compileError("index out of bounds!"),
        }
    }

    pub fn angleToVec(radians: f32, length: f32) Vec4 {
        return .{ .x = math.cos(radians) * length, .y = math.sin(radians) * length };
    }

    pub fn orthogonal(self: Vec4) Vec4 {
        return .{ .x = -self.y, .y = self.x };
    }

    pub fn add(self: Vec4, other: Vec4) Vec4 {
        return .{ .x = self.x + other.x, .y = self.y + other.y, .z = self.z + other.z, .w = self.w + other.w  };
    }

    pub fn subtract(self: Vec4, other: Vec4) Vec4 {
        return .{ .x = self.x - other.x, .y = self.y - other.y, .z = self.z - other.z, .w= self.w - other.w  };
    }

    pub fn scale(self: Vec4, s: f32) Vec4 {
        return .{ .x = self.x * s, .y = self.y * s, .z = self.z * s, .w = self.w * s};
    }

    pub fn clamp(self: Vec4, min: Vec4, max: Vec4) Vec4 {
        return .{ .x = math.clamp(self.x, min.x, max.x), .y = math.clamp(self.y, min.y, max.y), .z = math.clamp(self.z, min.z, max.z) };
    }

    pub fn angleBetween(self: Vec4, to: Vec4) f32 {
        return math.atan2(f32, to.y - self.y, to.x - self.x);
    }

    pub fn distanceSq(self: Vec4, v: Vec4) f32 {
        const v1 = self.x - v.x;
        const v2 = self.y - v.y;
        return v1 * v1 + v2 * v2;
    }

    pub fn distance(self: Vec4, v: Vec4) f32 {
        return math.sqrt(self.distanceSq(v));
    }

    pub fn perpindicular(self: Vec4, v: Vec4) Vec4 {
        return .{ .x = -1 * (v.y - self.y), .y = v.x - self.x };
    }
};

test "Vec4 tests" {
    const v = Vec4{ .x = 1, .y = 5 };
    const v2 = v.orthogonal();
    const v_orth = Vec4{ .x = -5, .y = 1 };

    std.testing.expectEqual(v2, v_orth);
    std.testing.expect(math.approxEq(f32, -2.55, v.angleBetween(v2), 0.01));
    std.testing.expect(math.approxEq(f32, 52, v.distanceSq(v2), 0.01));
    std.testing.expect(math.approxEq(f32, 7.21, v.distance(v2), 0.01));
    std.testing.expect(math.approxEq(f32, -6, v.perpindicular(v2).y, 0.01));
}